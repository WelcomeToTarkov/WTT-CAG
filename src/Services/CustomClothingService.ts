...
export class CustomClothingService {
    ...
    private handleLocale(clothingConfig: ClothingConfig, clothingId: string): void 
    {
        for (const localeID in this.instanceManager.database.locales.global) 
        {
            ...
            try 
            {
                const itemKey = `${clothingId}`;
                const nameKey = `${clothingId} Name`;
                const descriptionKey = `${clothingId} Description`;
    
                // Get locale data (fallback to 'en' if needed)
                const localeData = clothingConfig.locales[localeID] || clothingConfig.locales["en"];
                
                if (localeData) 
                {
                    // Handle both old (string) and new (object) format
                    const nameValue = typeof localeData === 'string' 
                        ? localeData 
                        : localeData.ClothingName;
                    
                    const descriptionValue = typeof localeData === 'string'
                        ? ''  // Default empty description for old format
                        : localeData.Description;
    
                    // Set name and description locales
                    this.instanceManager.database.locales.global[localeID][itemKey] = nameValue;
                    this.instanceManager.database.locales.global[localeID][nameKey] = nameValue;
                    this.instanceManager.database.locales.global[localeID][descriptionKey] = descriptionValue;
                }
            }
            catch (error) 
            {
